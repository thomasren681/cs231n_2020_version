...
def content_loss(content_weight, content_current, content_original):
    ...
    square_sum = torch.sum((content_original-content_current)**2)
    loss = square_sum*content_weight
...
def gram_matrix(features, normalize=True):
    ...
    # print(N, C, H, W)
    features = features.view(N, C, -1)# has shape (N, C, M=H*W)
    # print(features.shape)
    features_transpose = features.permute(0, 2, 1)#has shape (N, M, C)
    gram = torch.matmul(features, features_transpose)
    # print(gram.shape)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        idx = style_layers[i]
        styleloss += content_loss(style_weights[i], gram_matrix(feats[idx]), style_targets[i])
...
def tv_loss(img, tv_weight):
    ...
    first_term = torch.sum((img[:,:,1:,:]-img[:,:,:-1,:])**2)
    second_term = torch.sum((img[:,:,:,1:]-img[:,:,:,:-1])**2)
    loss = tv_weight*(first_term+second_term)
