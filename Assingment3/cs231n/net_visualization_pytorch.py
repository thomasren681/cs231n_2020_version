...
def compute_saliency_maps(X, y, model):
    ...
    scores = model(X)
    #get the loss to compute gradients
    ...
    #get the saliency from the gradients
    saliency = X.grad.data
    saliency = saliency.abs()
    saliency, idx = torch.max(saliency, dim=1)
...
def make_fooling_image(X, target_y, model):
    ...
    #     target_x = scores[0, target_y]
    #     target_x.backward(torch.FloatTensor([1.0]))
    #
    ...
    for epoch in range(100):
        ...
        # 使用.data只改变X_fooling的值，并不影响其梯度；否则X_fooling作为中间节点时
        # 会导致X_fooling.grad的清除
        X_fooling.data += dX
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    scores = model(img)
    loss = scores[0][target_y] - l2_reg*torch.sum(img**2).item()
    loss.backward()
    img.data += learning_rate*img.grad.data/img.grad.data.norm()
    img.grad.data.zero_()

    return scores, loss, img
