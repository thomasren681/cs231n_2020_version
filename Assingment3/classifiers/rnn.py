...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        i.e. the image features are input feature and the captions are labels to compute the loss
        ...
        #     i.e. from feature(N,D) to initial hidden state(N,H)
        # (2) Use a word embedding layer to transform the words in captions_in     #
        #     from indices to vectors, giving an array of shape (N, T, W).         #
        #     i.e. transform the idx vector to a one-hot matrix
        #     (N,T) --> (N,T,W)
        # (3) Use either a vanilla RNN or LSTM (depending on self.cell_type) to    #
        #     process the sequence of input word vectors and produce hidden state  #
        #     vectors for all timesteps, producing an array of shape (N, T, H).
        #     i.e. using an RNN to encode the sequence
        #     (N,T,W) --> (N,T,H)
        # (4) Use a (temporal) affine transformation to compute scores over the    #
        #     vocabulary at every timestep using the hidden states, giving an      #
        #     array of shape (N, T, V).
        #     i.e. use sigmoid affine layer to convert sequence to probability
        # (5) Use (temporal) softmax to compute loss using captions_out, ignoring  #
        #     the points where the output word is <NULL> using the mask above.     #
        #     i.e. compute the probability with groundtruth by softmax loss
        #                                                                          #
        ...
        if self.cell_type == 'rnn':
            hiddens, cache_h = rnn_forward(captions_emb, features_proj, Wx, Wh, b)
        else:
            hiddens, cache_h = None, None
        ...
        if self.cell_type == 'rnn':
            demb, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dhiddens, cache_h)
        else:
            demb, dh0 = None,None
