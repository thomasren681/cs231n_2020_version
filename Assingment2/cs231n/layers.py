...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        # # normalize the x and using the shift and scale to get the output
        # out = gamma*x_hat + beta
        #
        # running_mean = momentum*running_mean + (1-momentum)*x_mean
        # running_var = momentum*running_var + (1-momentum)*x_var
        #
        # cache = (x, x_mean, x_var, x_hat, eps, gamma, beta)
...
def batchnorm_backward(dout, cache):
    ...
    # x, x_mean, x_var, x_hat, eps, gamma, beta = cache#get the stored value from cache
    # dgamma = np.sum(dout*x_hat, axis=0)
    # dbeta = np.sum(dout, axis=0)
    # dx_hat = dout*gamma
    # dx_var = -0.5*((x_var + eps)**-1.5)*np.sum(dx_hat*(x-x_mean), axis=0)
    # dx_mean = np.sum(dx_hat*(x-x_mean), axis=0)*-1 + dx_var*np.mean(-2*(x-x_mean), axis=0)
    #
    # N = x.shape[0]
    # dx = np.sum(dx_hat*(x-x_mean), axis=0) + dx_var*2*(x-x_mean)/N + dx_mean/N
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = dx_hat * a + np.sum(dx_hat * (x - batch_mean) * (-0.5) * (a ** 3), axis=0) * 2 * (x - batch_mean) / N + np.sum(
        - dx_hat * a, axis=0) / N
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        mask = (np.random.rand(*x.shape)<p) / p #create the mask for drop out
        out = x*mask#drop random entries from x
        ...
        out = x
        #another way is by dividing p on the mask and out is equal to x
        #either way works, but the element multiply will cost too much computation
...
def dropout_backward(dout, cache):
    ...
    if mode == "train":
        ...
        dx = dout*mask # only back feed those entries that not being dropped
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    #zero padding the image, the num_data and RGB channel don't need to pad

    out_H = 1+(H + 2*pad - HH)//stride
    out_W = 1+(W + 2*pad - WW)//stride
    #compute the output size of the image

    out = np.zeros((N, F, out_H, out_W))

    #using for loop , np.sum and element mutiplication to do the convolution operation
    ...
    #add the bias term
    out += b.reshape((1,F,1,1))#different bias for different channel
...
def conv_backward_naive(dout, cache):
    ...
    db = np.sum(dout, axis=(0,2,3))#bias is different between channels

    for i in range(out_H):
        for j in range(out_W):
            mask = x_pad[:,:, i*stride:i*stride+HH, j*stride:j*stride+WW]
            for k in range(F):
                dout_single = dout[:, k:k+1, i:i+1, j:j+1]
                dw[k, :, :, :] += np.sum(dout_single*mask, axis=0)
                dx_pad[:,:,i*stride:i*stride+HH, j*stride:j*stride+WW] += dout_single*w[k,:,:,:]
    #compute the gradient element-wise
    #noted that a single entry in dout covers a mask in x_pad

    dx = dx_pad[:,:,pad:-pad, pad:-pad]
    # #crop the dx from dx_pad
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(out_H):
        for j in range(out_W):
            mask = x[:,:,i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
            max_mask = np.max(mask, axis=(2,3), keepdims=True)
            idx = (max_mask == mask)
            dx[:,:,i*stride:i*stride+pool_height, j*stride:j*stride+pool_width] += idx*dout[:,:
                                                                                       , i:i+1,
                                                                                       j:j+1]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    input = x.transpose(0,2,3,1).reshape(-1,C)#stretch the spatial matrix to a 2-D tensor
    two_D_out, cache = batchnorm_forward(input, gamma, beta, bn_param)
    #then reshape this tensor back to a spatial matrix
    out = two_D_out.reshape(N, H, W, C).transpose(0, 3, 1, 2)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # N, C, H, W = x.shape
    # x_g = x.reshape(N*G, -1)
    # mean = np.mean(x_g, axis=1).reshape(-1,1)
    # var = np.var(x_g, axis=1).reshape(-1,1)
